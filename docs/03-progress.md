...
- **Decisions:** D-027.
- **Next:** user tests logo upload in settings + printed bon with logo on WD8007. Then POS receipt text-mode conversion if bon path proves solid.

### 2026-08-29 — Mobile: Arabic print → transliteration (text mode only, raster dropped)
- **Step:** Comprehensive Mac-side hardware test: sent Arabic text to WD8007 via BT serial (`/dev/cu.CS3-D32E`) in all 48 ESC/POS codepages, CP1256 visual/logical, and UTF-8. Zero Arabic codepages rendered readable text — firmware has no usable Arabic support. Codepage 40 showed separated Arabic glyphs (closest match) but no shaping. Raster path (CoreText → bitmap → `GS v 0` / `ESC *`) renders perfect Arabic images but BLE serial is too slow/unreliable for the 10KB+ payloads (hangs, buffer overflows). New approach: `mobile/src/services/printer/transliterate.ts` — Arabic → French phonetic Latin transliteration (letter map, al- prefix, shadda doubling, Arabic-Indic → Western digits). `bonNeedsRaster()` now always returns `false`. `buildBonTicketLines` transliterates all user-facing strings (businessName, partyName, product labels, UI labels). All printing uses fast ESC/POS text mode regardless of language.
- **Result:** ✅ mobile jest 45/45 (updated `bonNeedsRaster` tests → always-false, added 5 transliteration tests). Device print pending — user to verify transliterated Arabic on WD8007.
- **Decisions:** D-026.
- **Next:** user tests transliterated bon print on device. Then POS receipt text-mode conversion if bon path proves solid.

### 2026-08-28 — Mobile: hybrid bon print — Arabic content routes to raster, Latin to text ticket (device scan verdict)
- **Step:** User ran the 48-table code scan + Arabic test on the WD8007. Verdict: firmware has Arabic glyph tables (n=10/20/22 print connected Arabic glyphs) but none render our cp1256 byte text readably — glyph soup in every mode; text-mode Arabic on this firmware is a dead end. Solution: `bonNeedsRaster()` in `mobile/src/services/printer/bon-ticket.ts` (+ `hasArabic()` in `arabic.ts`) — bon prints as raster bitmap when UI lang is `ar` or any of businessName/partyName/line labels contain Arabic (system font shapes perfectly; threshold-200 fix makes raster reliable); fully-Latin bons keep the fast ESC/POS text path. `PrintableBon` restored from git, both bon screens (`app/bons/[id].tsx`, `app/bons/index.tsx`) wired for both paths. Debug screen's Arabic test rewritten: one paper, three labeled sections (`== PLAIN == / == LOGICAL == / == UTF8 ==`) of the same sample, so a future printer is calibrated with a single print instead of three.
- **Result:** ✅ mobile jest 60/60 (+3 `bonNeedsRaster` tests), `tsc --noEmit --ignoreDeprecations 6.0` error set unchanged from baseline (5 pre-existing, incl. restored `PrintableBon`'s `writingDirection` which existed at HEAD), eslint 0 errors (3 pre-existing warnings). Device print of a real Arabic bon pending.
- **Decisions:** D-025.
- **Next:** user prints bon BC-2026-0001 (Arabic client/product) — should come out via raster with connected Arabic. Then optionally same hybrid for POS receipts.
- **Addendum (same day, later):** device re-test — all three text modes (`plain`/`logical`/`utf8`) print glyph soup on the WD8007 (UTF-8 section shows 2 glyphs per char = no UTF-8 decoder); raster checkerboard test prints fine. Web research confirms the community best practice: render via OS text engine → 1-bit bitmap → `GS v 0` (arabic-reshaper/text-mode hacks "break on mixed text"; whole-ticket raster recommended). Hybrid routing already implements exactly that. Added Printer Diagnostics → **"Arabic raster test (image path)"**: renders a real `PrintableBon` (الجويد/ادوكا sample, lang=ar) through the exact raster flow bons take — one-tap proof without a real bon. jest 60/60, tsc baseline unchanged, eslint clean.
- **Addendum 2 (same day):** user reports raster Arabic bon also fails on device. Built PC-side test bench `mobile/tools/printer-test.html` — Web Bluetooth (Chrome/macOS): connect WD8007, manual service/char UUID entry, graduated tests (feed / ASCII / GS v 0 checkerboard / ESC * checkerboard / canvas-rendered Arabic+French image through both image modes, threshold + font-size + chunk-size controls, live preview) — probes printer capability directly, bypassing the app. App Diagnostics also gains "Simple image test" (minimal Latin+Arabic capture) between the raw checkerboard and the full bon raster test, so capture-path vs payload failures are bisectable. jest 60/60, tsc/eslint clean. PC bench run pending — user connects printer to Mac, tests T1→T5.

### 2026-08-28 — Mobile: bon print moves to ESC/POS text mode + Arabic cp1256 encoding (supersedes same-day raster fix)
- **Step:** User test of the morning's raster fix still printed only the two dashed separators on the WD8007 — bitmap arrives, but the 576-dot downscale leaves glyph strokes anti-aliased and the `gray < 128` threshold erased them. Per user direction, bon printing now bypasses bitmaps entirely: new `mobile/src/services/printer/arabic.ts` (RTL visual reordering, digits kept LTR, Windows-1256 encoder), `text.ts` (ESC/POS text ticket: ESC a/ESC ! align+bold+double-size, 48-col two-column rows, dashed rules), `bon-ticket.ts` (bon data → ticket lines, labels injected); `usePrinter.printTicket(payload)` (reset/feed/cut wrapped, ensureConnected); bon detail + list screens print via the text path (`PrintableBon` deleted, `PrintPreview`/raster remain for POS receipts + POs). Printer Diagnostics gains Arabic test print, mode picker (`plain` visual / `logical` printer-shaped / `utf8`), `ESC t` table setter (auto or 0–255) and a 48-sample code-table scan (D-024). Threshold raised 128 → 200 in `bitmap.ts` so the raster path prints text too.
- **Result:** ✅ mobile jest 57/57 (was 25; +32 new: arabic reorder/encode, ticket byte layout, bon mapping incl. RTL mirroring), `tsc` error set unchanged from baseline (4 pre-existing in untouched files), eslint 0 errors. Device print pending — user runs Arabic test + code-table scan on the WD8007.
- **Decisions:** D-024.
- **Next:** user prints: (1) Arabic test in current mode; if garbled, scan tables and pick the `n=` that renders Arabic, set mode that reads best; if no table renders Arabic, fall back to raster mode (threshold fix) which keeps system-font Arabic shaping. Then convert POS receipts to text tickets if the bon path proves solid.
